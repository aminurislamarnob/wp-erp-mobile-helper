...
(function ( $ ) {
    ...
    function showFormSuccess( isEdit ) {
        var title = isEdit ? erpPaymentRequest.i18n.successTitleEdit : erpPaymentRequest.i18n.successTitle;
        var msg   = isEdit ? erpPaymentRequest.i18n.successMessageEdit : erpPaymentRequest.i18n.successMessage;
    ...
    $( document ).on( 'click', '#erp-pr-form-modal-close, #erp-pr-form-modal-cancel', function ( e ) {
        e.preventDefault();
        closeFormModal();
    } );
        ...
        if ( ! mediaFrame ) {
            ...
            mediaFrame.on( 'select', function () {
                mediaFrame.state().get( 'selection' ).each( function ( attachment ) {
                    ...
                    var ext = att.filename ? att.filename.split( '.' ).pop().toLowerCase() : '';

                    if ( ALLOWED_EXTS.indexOf( ext ) === -1 ) {
                        showFormError( erpPaymentRequest.i18n.invalidType.replace( '{name}', att.filename ) );
                        return;
                    }
                    if ( ( att.filesizeInBytes || 0 ) > MAX_FILE_SIZE ) {
                        showFormError( erpPaymentRequest.i18n.fileTooLarge.replace( '{name}', att.filename ) );
                        return;
                    }
    ...
    function renderAttachment( id, filename ) {
        ...
        $( '#erp-pr-attachment-list' ).append(
            '<div class="erp-pr-attachment-item" id="erp-pr-attachment-' + id + '">' +
                '<input type="hidden" name="attachment_ids[]" value="' + id + '">' +
                '<span class="dashicons dashicons-paperclip"></span>' +
                '<span class="erp-pr-attachment-name">' + filename + '</span>' +
                '<button type="button" class="erp-pr-remove-attachment" data-id="' + id + '">&times;</button>' +
            '</div>'
        );
    }
    ...
    $( document ).on( 'click', '#erp-pr-submit-btn', function ( e ) {
        ...
        if ( ! $( '#erp-pr-title' ).val().trim() ) {
            showFormError( erpPaymentRequest.i18n.fieldRequired.replace( '{field}', 'Title' ) );
            return;
        }
        if ( ! ( parseFloat( $( '#erp-pr-amount' ).val() ) > 0 ) ) {
            showFormError( erpPaymentRequest.i18n.fieldRequired.replace( '{field}', 'Amount' ) );
            return;
        }
        if ( ! $( '#erp-pr-description' ).val().trim() ) {
            showFormError( erpPaymentRequest.i18n.fieldRequired.replace( '{field}', 'Description' ) );
            return;
        }
        ...
        var action   = isEdit ? 'erp_app_helper_update_payment_request' : 'erp_app_helper_submit_payment_request';
        ...
        $.post( erpPaymentRequest.ajaxurl, formData )
            .done( function ( response ) {
                if ( response.success ) {
                    ...
                    $btn.prop( 'disabled', false ).text( isEdit ? erpPaymentRequest.i18n.saveChanges : erpPaymentRequest.i18n.submit );
                }
            } )
            .fail( function () {
    ...
    $( document ).on( 'click', '.erp-pr-approve-btn', function ( e ) {
        ...
        $( '#erp-pr-approve-modal-submit' ).prop( 'disabled', false ).text( erpPaymentRequest.i18n.confirmApproval );
        $( '#erp-pr-approve-modal' ).fadeIn( 200 );
    } );

    $( document ).on( 'click', '#erp-pr-approve-modal-close, #erp-pr-approve-modal-cancel', function ( e ) {
        e.preventDefault();
        ...
        $.post( erpPaymentRequest.ajaxurl, {
            ...
            .done( function ( response ) {
                if ( response.success ) {
                    ...
                    $btn.prop( 'disabled', false ).text( erpPaymentRequest.i18n.confirmApproval );
                }
            } )
            .fail( function () {
    ...
    $( document ).on( 'click', '#erp-pr-modal-close, #erp-pr-modal-cancel', function ( e ) {
        e.preventDefault();
        ...
        $.post( erpPaymentRequest.ajaxurl, {
            ...
            .done( function ( response ) {
                if ( response.success ) {
                    ...
                    alert( response.data || erpPaymentRequest.i18n.error );
                    $btn.prop( 'disabled', false ).text( erpPaymentRequest.i18n.submitRejection );
                }
            } )
            .fail( function () {
                alert( erpPaymentRequest.i18n.error );
